...
algorithms = ['bfs','obtea','opt_h0_llm','uhbtp','opt_h0']  # , 'opt_h1','opt_h1_llm'
algorithms_name ={
    'bfs':'BT Expansion',
    'obtea':'OBTEA',
    'opt_h0_llm':'HBTP',
...
scenes = ['RH', 'VH', 'OG', 'RW']  # 'RH', 'VH', 'OG', 'RW'
...
for scene in scenes:
    for alg_str in algorithms:
        ...
        data_path_single = os.path.join(os.path.dirname(__file__), f"algo_details_t=1/single_{scene}_{alg_str}.csv")
        data_path_multi = os.path.join(os.path.dirname(__file__), f"algo_details_t=1/multi_{scene}_{alg_str}.csv")
...
# 将结果保存到 CSV 文件
results_df.to_csv(os.path.join(os.path.dirname(__file__), 'algorithm_performance_results.csv'), index=False)
...
results_df_sorted = results_df.sort_values(
    by=['Scene'],
    key=lambda col: col.map({'RW': 0, 'VH': 1, 'OG': 2, 'RH': 3})
)
...
{vh_rows}
\\midrule
\\multicolumn{{7}}{{l}}{{\\textbf{{Scenario: OminiGibson}}}} \\\\
{og_rows}
\\midrule
...
og_rows = create_latex_rows(results_df_sorted, 'OG')
