...
# _,current_big_actions_name = filter_actions(data, big_actions)
# # 再把动作空间，扩展到 300 个
# current_big_actions = generate_custom_action_list(big_actions, 300, current_big_actions_name)
current_big_actions=big_actions
    ...
    for mode in ["user-defined","small-predicate-objs"]:
        for heuristic_choice in [-1]:
            ...
            algo = BTExpInterface(None, cur_cond_set=cur_cond_set,
                                  ...
                                  key_objects=key_obj, \
                                  selected_algorithm=selected_algorithm, mode=mode, \
...
results_df.to_csv(f"EXP_BT_3_big_small_3algo_table_001_record_timeout_bfs.csv", index=False)
print(f"Data has been saved to 'EXP_BT_3_big_small_3algo_table_001_record_timeout_bfs.csv'")
