...
for round_num in range(max_round):
    ...
    if round_num!=0:
        ...
        for n, chosen_goal in enumerate(round_goals):
            ...
            tarin_result = perform_test(env, chosen_goal, database_index_path,train=True)
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(validate_goal, env, d['Goals'], database_index_path, round_num, n, database_num) for n, d in enumerate(vaild_dataset_test)]
...
df.to_csv(f'EXP_2_DATABSE_random_round={max_round}_sample={sample_num}.csv', index=False)
...
plt.savefig(f'EXP_2_DATABSE_success_rate_random_round={max_round}_sample={sample_num}.png')
...
plt.savefig(f'EXP_2_DATABSE_similarity_random_round={max_round}_sample={sample_num}.png')
