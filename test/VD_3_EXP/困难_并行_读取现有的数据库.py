...
def extract_metadata_from_file(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        for line in file:
            ...
            if line.isdigit():
                if current_metadata:
                    metadata_list.append({'value': current_metadata})
                current_metadata = {'Environment': line}
            elif line.startswith("Goals:"):
                current_metadata['Goals'] = line.split(":", 1)[1].strip()
            elif line.startswith("Optimal Actions:"):
                current_metadata['Optimal Actions'] = line.split(":", 1)[1].strip()
            elif line.startswith("Vital Action Predicates:"):
                current_metadata['Vital Action Predicates'] = line.split(":", 1)[1].strip()
            elif line.startswith("Vital Objects:"):
                current_metadata['Vital Objects'] = line.split(":", 1)[1].strip()
        if current_metadata:  # Add the last read metadata
            metadata_list.append({'value': current_metadata})

    return metadata_list


def save_metadata_as_npy(metadata, output_file_path):
    np.save(output_file_path, metadata, allow_pickle=True)
...
for round_num in range(0, 0 + max_round):
    ...
    file_path  = f"{ROOT_PATH}/../test/VD_3_EXP/DATABASE/{name}/TMP_Zdatabase_{name}_Det_round{round_num}_mr=21_smpl=5.txt"
    database_index_path = f"{ROOT_PATH}/../test/VD_3_EXP/DATABASE/{name}/TMP_Zdatabase_{name}_Det_round{round_num}_mr=21_smpl=5.index"
    # output_path =  f"{ROOT_PATH}/../test/VD_3_EXP/DATABASE/{name}/TMP_Zdatabase_{name}_Det_round{round_num}_mr=21_smpl=5_metadata.npy"
    # write_metadata_to_txt(database_index_path, output_path)
    # metadata = np.load(database_index_path.replace(".index", "_metadata.npy"), allow_pickle=True)

    metadata_output_path = file_path.replace(".txt", "_metadata.npy")
    metadata = extract_metadata_from_file(file_path)
    save_metadata_as_npy(metadata, metadata_output_path)

    database_num = len(metadata)
    print(f"\033[91m -----------  Round: {round_num}  Database Num: {database_num} -----------\033[0m")
