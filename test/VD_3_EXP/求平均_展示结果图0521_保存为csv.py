...
if not mean_data or not std_data:
    ...
    # Initialize dictionary to hold results for saving to CSV
    ...
    for metric in metrics:
        ...
        for name in names:
            if name in filtered_mean_data and name in filtered_std_data:
                ...
                metric_data.append(pd.DataFrame({
                    'Round': filtered_rounds,
                    f'{name} Mean': smoothed_mean_values,
                    f'{name} Std': smoothed_std_values
                }))
        ...
        # Combine data for this metric and save to CSV
        combined_metric_df = pd.concat(metric_data, axis=1)
        combined_metric_df = combined_metric_df.loc[:,~combined_metric_df.columns.duplicated()]  # Remove duplicate 'Round' columns
        combined_metric_df.to_csv(f'results/{metric.replace(" ", "_")}_mean_std.csv', index=False)
