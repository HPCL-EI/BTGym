...
from btgym.utils.tools import collect_action_nodes, read_env_file
...
# 直接读入 env=1 的数据
file_name = "random_env_dataset(75×4)"
...
env_file_name = f"{ROOT_PATH}/../test/dataset/ENV_5.txt"
env_dict = read_env_file(env_file_name)
# 示例：检索 key="1" 的环境设置
key = "1"
if key in env_dict:
...
for id, d in enumerate(data1):
    ...
    # 修改环境格式信息
    cur_cond_set = env.agents[0].condition_set = env_dict[d["Environment"]]
    cur_cond_set |= {"IsRightHandEmpty(self)", "IsLeftHandEmpty(self)", "IsStanding(self)"}
...
print("---------------")
print(
    f"Total errors: {len(errors)}, Indices: {[error['id'] for error in errors]}, Goals: {[error['Goals'] for error in errors]}")
print(
    f"Total time limits exceeded: {len(time_limits_exceeded)}, Indices: {[tl['id'] for tl in time_limits_exceeded]}, Goals: {[tl['Goals'] for tl in time_limits_exceeded]}")
print("---------------")
