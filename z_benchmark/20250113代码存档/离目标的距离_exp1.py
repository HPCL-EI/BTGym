...
def plot_percentage(percentages_type, difficulty, scene, algo_type, max_epoch, data_num, save_csv=False):
    ...
    for algo_str in algo_type:  # "opt_h0", "opt_h1", "obtea", "bfs"
        ...
        if save_csv == True:
            ...
            file_path = f'{ROOT_PATH}/../z_benchmark/percentage_output/{percentages_type}_{difficulty}_{scene}_{algo_str_complete}.csv'
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'{ROOT_PATH}/../z_benchmark/percentage_images/{percentages_type}_{difficulty}_{scene}.png', dpi=100)
    # plt.show()
